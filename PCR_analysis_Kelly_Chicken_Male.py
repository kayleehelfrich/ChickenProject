...
#FILE INPUTS
print "File Inputs:"
PCR_file1 = 'Blk1_4.AvUCP.PreHeat.Sex.Diet.Inc_CLEAN_Male.csv'
print "FILE 1:", PCR_file1
PCR_file2 = 'Blk5_8.AvUCP.PreHeat.Sex.Diet.Inc_CLEAN_Male.csv'
print "FILE 2:", PCR_file2
PCR_file3 = 'Blk9_12.AvUCP.PrePostHeat.Sex.Diet.Inc_CLEAN_Male.csv'
print "FILE 3:", PCR_file3
PCR_file4 = 'Blk13-16.AvUCP.PostHeat.Sex.Diet.Inc_CLEAN_Male.csv'
print "FILE 4:", PCR_file4
PCR_file5 = 'Blk16-20.AvUCP.PostHeat.Sex.Diet.Inc_CLEAN_Male.csv'
...
def Ct_calculations_print(sampleAll, set_nameAll, CqAvgAll, CqDevAll, fileNum):
    ...
    print df_sort
    fileName = 'Blks1-20.AvUCP_Male_Cq_calculations_'+str(fileNum)+'.csv' #make a CSV file out of the dataframe unique to the file number (fileNum)
...
def sem_calculation(set_names_merge, means):
    ...
    sets = ['MpreA', 'MpreB', 'MpreC', 'MpreD', 'MpreE', 'MpreF', 'MpostA', 'MpostB', 'MpostC', 'MpostD', 'MpostE', 'MpostF'] #MAKE SURE CALIBRATOR IS FIRST VALUE IN THE LIST
    MpreA = []
    MpreB = []
    MpreC = []
    MpreD = []
    ...
    for i in range(0, len(set_names_merge)): #loop through and add to each set it's corresponding values
        if set_names_merge[i] == 'MpreA':
            MpreA.append(means[i])
        if set_names_merge[i] == 'MpreB':
            MpreB.append(means[i])
        if set_names_merge[i] == 'MpreC':
            MpreC.append(means[i])
        if set_names_merge[i] == 'MpreD':
            MpreD.append(means[i])
        if set_names_merge[i] == 'MpreE':
            MpreE.append(means[i])
        if set_names_merge[i] == 'MpreF':
            MpreF.append(means[i])
        if set_names_merge[i] == 'MpostA':
            MpostA.append(means[i])
        if set_names_merge[i] == 'MpostB':
            MpostB.append(means[i])
        if set_names_merge[i] == 'MpostC':
            MpostC.append(means[i])
        if set_names_merge[i] == 'MpostD':
            MpostD.append(means[i])
        if set_names_merge[i] == 'MpostE':
            MpostE.append(means[i])
        if set_names_merge[i] == 'MpostF':
            MpostF.append(means[i])
    #taking the mean of each set
    MpreA_avg = sum(MpreA)/len(MpreA)
    MpreB_avg = sum(MpreB)/len(MpreB)
    MpreC_avg = sum(MpreC)/len(MpreC)
    MpreD_avg = sum(MpreD)/len(MpreD)
    MpreE_avg = sum(MpreE)/len(MpreE)
    MpreF_avg = sum(MpreF)/len(MpreF)
    MpostA_avg = sum(MpostA)/len(MpostA)
    MpostB_avg = sum(MpostB)/len(MpostB)
    MpostC_avg = sum(MpostC)/len(MpostC)
    MpostD_avg = sum(MpostD)/len(MpostD)
    MpostE_avg = sum(MpostE)/len(MpostE)
    MpostF_avg = sum(MpostF)/len(MpostF)
    #averages combined into list
    set_averages.append(MpreA_avg)
    set_averages.append(MpreB_avg)
    set_averages.append(MpreC_avg)
    set_averages.append(MpreD_avg)
    set_averages.append(MpreE_avg)
    set_averages.append(MpreF_avg)
    set_averages.append(MpostA_avg)
    set_averages.append(MpostB_avg)
    set_averages.append(MpostC_avg)
    set_averages.append(MpostD_avg)
    set_averages.append(MpostE_avg)
    set_averages.append(MpostF_avg)
    #SEMs calculated and appended to list
    SEMs.append(stats.sem(MpreA))
    SEMs.append(stats.sem(MpreB))
    SEMs.append(stats.sem(MpreC))
    SEMs.append(stats.sem(MpreD))
    SEMs.append(stats.sem(MpreE))
    SEMs.append(stats.sem(MpreF))
    SEMs.append(stats.sem(MpostA))
    SEMs.append(stats.sem(MpostB))
    SEMs.append(stats.sem(MpostC))
    SEMs.append(stats.sem(MpostD))
    SEMs.append(stats.sem(MpostE))
    SEMs.append(stats.sem(MpostF))
...
df_final.to_csv('Male_Chicken_FoldChanges.csv') #send dataframe to a file
