...
library(ggplot2)
#library(ggsignif)
setwd("C:/Users/khelfri/Documents/KayleeStuff/Smith_Lab/Data/Kelly_qPCR")

### Reads a file in a table format and creates a data frame from it. This figures will be in the first panel.
datafile <- "Blks1-20.AvUCP_Female_Cq_calculations_COMBINED_For_ddCtPlot.csv"
...
order <- c("FpreA", "FpreB", "FpreC", "FpreD", "FpreE", "FpreF", "FpostA", "FpostB", "FpostC", "FpostD", "FpostE", "FpostF")

## Create a dot plot overlaid on a bar chart
# Initialize an individual high resolution graph
png("Female_Chicken_AvUCP_Plot.png", units="mm", width=300, height=200, res=600)
...
plot <- ggplot(data= dataframe_AdjCtCalcs, aes(x=Biological.Sets, y=mean), fill=order) + #creates original graph
  ...
  ggtitle("AvUCP Expression in Females") +
  ...
  scale_y_continuous(limits = c(-2,12.0)) + #, expand = c(0,0)) + #this sets the minimum and maximum values shown on the y-axis
  ylab(expression("Female AvUCP/Gapdh Relative to Group Average")) + #adds y-axis label
  ...
  theme(
    plot.title = element_text(hjust = .5, size = 24), #sets size and position of graph title
